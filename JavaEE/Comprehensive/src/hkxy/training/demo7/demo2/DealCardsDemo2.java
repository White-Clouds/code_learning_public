package hkxy.training.demo7.demo2;

import java.util.*;

public class DealCardsDemo2 {
    ...
    private final List<String> playerD = new ArrayList<>();//玩家D
    List<String> cardList = new ArrayList<>();//随机牌组的链表

    public static void main(String[] args) {
        DealCardsDemo2 dealCards = new DealCardsDemo2();//初始化对象
        dealCards.dealCards();
        ...
        System.out.println("玩家D的牌是：");
        showCards(dealCards.getPlayerD());
    }
    ...
    public void dealCards() {
        ...
        } else if (playerC.contains(landLordCard)) {
            System.out.println("地主是玩家C");
            while (!bottomCards.isEmpty()) {
                playerC.add(bottomCards.removeFirst());
            }
        } else {
            System.out.println("地主是玩家D");
            while (!bottomCards.isEmpty()) {
                playerD.add(bottomCards.removeFirst());
            }
        }
    }
